...
// two honest actors race to register the same name; the loser (later effHeight) pays the reg fee.
...
// the salt within COMMIT_MAX_BLOCKS, back-dating below the victim → displaces them. Pre-V25 the victim
// paid the reg fee and loses the name (displacement burn). Post-V25 the victim only holds a payment-free
// reservation, so the displacement costs them nothing.
function blindCommitDisplace(h) {
  const nm = "bd" + ri(100, 9999);
  const salt = hexSalt(ri(16, 40));
  const fee = nameRegFee(nm, h).toString();
  const commitEv = prop(ACTORS.ADV, nameCommitRecord({ commit: nameCommit(nm, salt, ACTORS.ADV) }), h);
  const victimReg = prop(ACTORS.A, nameClaim({ name: nm }), h + ri(1, 5), { [T]: fee }); // honest direct register
  const advReveal = prop(ACTORS.ADV, nameClaim({ name: nm, salt }), h + ri(6, 30), { [T]: fee }); // back-dates to commit h
  return { label: `blind-commit-displace@${h}`, cls: "displacement", tipHeight: h + 60, events: [commitEv, victimReg, advReveal] };
}

// an offer plus competing fills and a same-block cancel-snipe. Exercises double-fill (payment into a
// filled offer) and the fill-before-cancel guard (a late cancel must not strand an in-flight fill).
function offerFillRace(h) {
  ...
  const val = "100000000";
  const off = prop(ACTORS.A, offer({ give: { ticker: tk, amount: "10" }, want: { value: val } }), h + 2, {}); // OPEN (v1.7) if ≥V17
  const fee = tradeFee(BigInt(val), h + 2 >= V16_HEIGHT ? FEE_BPS_V16 : 100).toString();
  const reb = h + 2 >= 34000 ? tradeFee(BigInt(val), 50).toString() : "0"; // v1.7 open-ask rebate approximation guard
  const pay = { [ACTORS.A]: val, [T]: fee };
  const ev = [dep, mnt, off];
  // claim (v1.7) then two competing fills in adjacent blocks + a late cancel that must not win
  if (chance(0.7)) ev.push(att(ACTORS.B, off.id, h + 3, {}, 50));
  ev.push(att(ACTORS.B, off.id, h + 4, pay, 100, 1));
  if (chance(0.6)) ev.push(att(ACTORS.C, off.id, h + 4, pay, 100, 2));         // same-block competing fill
  if (chance(0.5)) ev.push(prop(ACTORS.A, offerCancelAll({ ticker: tk }), h + 4, {}, 0)); // cancel-snipe (same block, out-pos)
  if (chance(0.4)) ev.push(att(ACTORS.B, off.id, h + 6, pay));                 // double-fill after settle
...
const GENS = [registerRace, blindCommitDisplace, offerFillRace, listThenTransfer];
